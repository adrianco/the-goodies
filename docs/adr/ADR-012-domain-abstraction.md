...
**Status:** Accepted · 2026-08-01 · Implemented in part 2026-09-13 — §1 (vocabulary in a manifest, enforced on every write path), §2 (declarative `DomainTool`s, skills named by the manifest; the five house tools moved out of the engine), §6.3 (`domains/vehicles` first pass: manifest, seed, eight tools, `vehicle-walk`). §3 is delivered as one process per domain and is now [ADR-018](ADR-018-multi-domain-hosting.md); §4 (cross-domain references) is promoted to [ADR-017](ADR-017-cross-domain-references.md) with temporal semantics and broken-reference handling; the vehicles domain itself is [ADR-016](ADR-016-vehicles-domain.md). See `docs/domains.md` §6. · Renamed 2026-08-03: the second domain is
...
throughout this ADR rather than noted only here, because the old name would
otherwise be copied into the package when it is written.

## Context

Owner direction: the sync/auth/query engine should serve **unrelated domains**. First: the existing smart-home graph. Second: a **car collection** — what vehicles exist, the history of each car, issues and service records. Topology decided up front: **shared auth/sync/query code · separate endpoint · separate database file · separate MCP client per domain.** Sequencing decided too: abstract first, get the new code working for house, then instantiate vehicles.
...
The vehicles domain is also a validation case: "history of the cars and issues" is *exactly* the as-of machinery — a car's bay changes are interval edges, an issue's lifecycle is version history, `snapshot(T)` answers "what was in the collection in March".
...
domains/
  ...
  vehicles/ — CAR, BAY, SERVICE_RECORD, ISSUE, DOCUMENT, ... (phase 2)
```
...
- **Storage:** one database file per domain (`house.db`, `vehicles.db`) — each keeps the single-file backup story (ADR-001/007); the backup/mirror jobs enumerate configured domains.
...
- **A reference is an ordinary interval edge that lives entirely in the referring domain**, whose remote endpoint is a qualified value: `(domain, entity_id)` plus an optional cached display label. The vehicles `stored_at` edge from the parts box to `house:⟨room-id⟩` is a vehicles row in `vehicles.db` — it syncs on vehicles' timeline, counts in vehicles' digest, and obeys vehicles' snapshot rule for its interval and its *local* endpoint. Nothing about it touches house's database, sequence, or digest.
...
3. **Instantiate:** `domains/vehicles` — manifest + seed + a handful of declarative tools (`get_cars_in_bay`, `get_car_history`, `get_open_issues`, `get_service_records`); second database file; second MCP client config.
...
- **Keep enums, add vehicles members to them** — one shared vocabulary pollutes both domains and makes the *third* domain worse; rejected on arrival.
